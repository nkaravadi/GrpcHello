export colima_host_ip=$(/sbin/ifconfig bridge100 | grep "inet " | cut -d' ' -f2)
...
export colima_vm_iface=$(colima ssh -- "/sbin/ifconfig" | grep -B 1 $colima_vm_ip | cut -d' ' -f1)
echo colima_vm_iface $colima_vm_iface
export colima_kind_iface=$(colima ssh -- "/sbin/ifconfig" | grep -B 1 $colima_kind_cidr_short | cut -d' ' -f1)
